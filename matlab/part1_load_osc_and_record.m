...
% (from the scene definition file), the test signal must only contain zeros:
...
% stimulus can be created as a matrix, with one column per
% entry in the variable c_Write and one row per audio sample.
...
% plot the recorded signal with whitenoise
hold on
